...
RUN LC_ALL=C DEBIAN_FRONTEND=noninteractive apt-get update \
    && LC_ALL=C DEBIAN_FRONTEND=noninteractive apt-get install -qqy --no-install-recommends \
        ...
        libimage-imlib2-perl=2.03-1+b8 \
        libimage-info-perl=1.41-1 \
        libimage-librsvg-perl=0.07-8+b6 \
...
RUN if [ "${IMAGE_LAYER_PERL_EXT}" != "0" ]; then \
      ...
      && LC_ALL=C DEBIAN_FRONTEND=noninteractive apt-get install -qqy --no-install-recommends \
        ...
        libauthen-krb5-admin-perl=0.17-1+b9 \
        libauthen-krb5-perl=1.9-5+b4 \
        libauthen-krb5-simple-perl=0.43-2+b7 \
        ...
        libauthen-sasl-cyrus-perl=0.13-server-10+b6 \
        ...
        libio-socket-multicast-perl=1.12-2+b5 \
